...
for line in ['accuracy', 'loss', 'rounds'] + \
    ...
    for non_iid in non_iids:
        ...
        idx += 1
        files = '../history/history_mnist_{}_fog_uniform_non_iid_{}_num_workers_{}_lr_{}_batch_{}_laplace_rounds_{}_radius_{}_d2d_{}_factor_{}_alpha_{}_dyn_{}_delta_{:.8f}_omega_{}_eps_mul_{}_kappa_{}.pkl'
        ...
        for i in range(len(kappas)):
            x_ax, y_ax, l_test, grad_tr, rounds_tr, _ = pkl.load(
                open(files.format(clf, non_iid, n, lr,
                                  ...
                                  dyn, delta, omega, eps_mul,
                                  kappas[i]), 'rb'))
            x_ax, y_ax, l_test = x_ax[:epochs], y_ax[:epochs], l_test[:epochs]
            if line == 'accuracy':
                ax.plot(x_ax, y_ax, colors[i],
                        label='kappa = ' + str(kappas[i]))
            elif line == 'loss':
...
file_name = '../plots/_mnist_{}_fog_uniform_non_iid_{}_num_workers_{}_lr_{}' \
            ...
            '_delta_{}_omega_{}_eps_mul_{}_kappas_{}'.format(
                clf, non_iid, n, lr, b, alpha, radius, d2d, factor,
                delta, omega, eps_mul, '_'.join(list(map(str, kappas)))
)
