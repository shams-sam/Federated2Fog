...
for line in ['accuracy', 'loss']:
    for non_iid in [1]:
        ...
        file_fl = '../history/history_mnist_svm_fl_uniform_non_iid_1_num_workers_200_lr_0.01_batch_300.pkl'
        file_15 = '../history/history_mnist_svm_fog_uniform_non_iid_1_num_workers_200_lr_0.01_batch_300_laplace_rounds_15_radius_0.2_d2d_1.0_factor_8.pkl'
        ...
        if line=='accuracy':
            ax.plot(x_ax, y_ax, 'k*-', label='EUT fogL')
        else:
            ax.plot(x_ax, np.array(l_test), 'k*-', label='EUT fogL')
        ...
        if line=='accuracy':
            ...
            ax.plot(x_ax, np.array(l_test)/10, 'ro:', label=r'$\theta$ = 1 with decaying lr')
        ...
        if line == 'accuracy':
            ...
            plt.plot(x_ax, np.array(l_test)/10, 'bs:', label=r'$\theta$ = 1 w/o decaying lr')
        ...
        if idx == 3:
                        ax.legend(loc='upper right', ncol=4, bbox_to_anchor=(-1.3, 1.1, 2.3, .125), mode='expand', frameon=False)
