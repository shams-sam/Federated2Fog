...
for line in ['accuracy', 'rounds']:
    for non_iid in non_iids:
        ...
        ax.set_ylabel(line)
        ax.set_xlabel('global synchronizations')
        ax.grid(True)
        ax.set_xlim(left=0, right=25)
        ax.set_title('({})'.format(title[idx]), y=-0.35)
        if idx == 2:
            ax.legend(loc='upper right', ncol=2, handlelength=1.5, bbox_to_anchor=(0.05, 1.10, 2.2, .27), mode='expand', frameon=False)
file_name = '../plots/{}_{}_fog_uniform_non_iid_{}_num_workers_{}_lr_{}' \
            '_batch_{}_laplace_alpha_{}_radius_{}_d2d_{}_factor_{}' \
            '_psis_{}_proposal1'.format(
